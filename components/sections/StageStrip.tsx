      ...
      {stages.map((stage, i) => {
        ...
        return (
          <li key={stage.percent} className="flex flex-col items-center gap-1.5 text-center">
            <div
              className={`relative aspect-square w-full overflow-hidden rounded-xl shadow-sm transition-transform duration-150 ${
                isGoal
                  ? 'bg-gradient-to-b from-[var(--soft)] to-[var(--accent)] ring-2 ring-[var(--primary)]'
                  : 'bg-gradient-to-b from-white to-[var(--pale)]'
              }`}
            >
              <Image
                src={stage.image}
